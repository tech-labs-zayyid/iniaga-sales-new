...
const ProductCard = ({ product, salesData }: any) => {
  ...
  const { product_name, price, city_name, province_name, best_product, images } = product;

  return (
    <Card className="py-0 pb-4 shadow-lg rounded-2xl w-full max-w-xs">
      ...
      <CardContent className="mt-2 space-y-2">
        ...
        <div className="w-full flex flex-col gap-2">
          <Button asChild className="w-full sm:w-auto">
            <a target="_blank" href={`https://wa.me/${salesData.phone_number}?text=Halo, Saya mau menanyakan produk ${product_name}`}>
              <PhoneCall className="size-4" />
              Tanya Produk
            </a>
          </Button>
          <Button asChild variant="outline" className="w-full sm:w-auto">
            <a href={`mailto:${salesData.email}`}>
              Detail Produk
            </a>
          </Button>
        </div>
